...
namespace MusicBeePlugin {
    public class Quiz {
        ...
        public void gameOverCheck(bool quickEnd) {
            ...
            if (!v.panel1.Visible) { //reveal at
                string pattern = "RevealAt: .*?(;|$)";
                ...
                if (re.IsMatch(line)) {
                    ...
                    startMS = startMS.Substring(9).TrimEnd(';');
            ...
            if (!v.HintPicture.Visible && !v.panel1.Visible) { //show hint at

                string pattern = "HintAt: .*?(;|$)";
                Regex re = new Regex(pattern);
                string path = (Environment.GetFolderPath(Environment.SpecialFolder.MyMusic) + "\\MusicBee\\QUIZ.txt");
                int index = v.mApi.NowPlayingList_GetCurrentIndex();
                string line = File.ReadLines(path).Skip(index).FirstOrDefault();
                ...
                if (re.IsMatch(line)) {
                    ...
                    if (success) {
                        if (A > i) {
                            ...
                            path = (Environment.GetFolderPath(Environment.SpecialFolder.MyMusic) + "\\MusicBee\\QUIZ.txt");
                            index = v.mApi.NowPlayingList_GetCurrentIndex();
                            line = File.ReadLines(path).Skip(index).FirstOrDefault();

                            string url = re2.Match(line).Value;
